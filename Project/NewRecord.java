...
//Date: June 15, 2016
//Description: NewRecords reads data from customers.txt and writes it back to the file with the new record.


import java.io.*;

/** NewRecords reads data from customers.txt and writes it back to the file with the new record.
...
class NewRecord
{
   ...
   * @param newRec      The String that is returned from KeyInput method that contains the String suitable for insertion    */ 
   public static void addRecord(String fileName, String newRec)
   {
      ...
      try 
      {
         DataInput f1 = new DataInputStream(new FileInputStream("customers.txt"));
         ...
         DataOutput f2 = new DataOutputStream(new FileOutputStream("customers.txt"));
